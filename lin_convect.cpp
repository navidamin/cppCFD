...
void linConvect(int dimension, const SimParams& inputParameters) {
    ...
    // create the output instance for writing out the results
    Output output;
    ...
    for (int n = 0; n < nt; ++n) {
        ...
        for (int i = 1; i < nx; ++i) {
            u(0, i) = un(0, i) - c * dt / dx * (un(0, i) - un(0, i-1));
        }
